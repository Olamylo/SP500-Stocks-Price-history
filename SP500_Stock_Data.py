...
with engine.begin() as con:
    sql_command1 = f"""SELECT MAX(Date) as max_date FROM STAGING_SP500_STOCK_PRICES;"""
...
# get next day date
next_day = maxdate + datetime.timedelta(days=1)
...
if yesterday > maxdate:
    ...
    df = get_date(data, bulkid, date_added)

    # df.to_csv('./stock_test.csv', index=False, encoding="utf-8")

    df.to_sql('STAGING_SP500_STOCK_PRICES', con=engine, if_exists='append', index=False)
